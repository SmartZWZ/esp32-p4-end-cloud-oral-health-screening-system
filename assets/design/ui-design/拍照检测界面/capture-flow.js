(() => {
  ...
  const captureModeSelect = document.querySelector('#capture-mode-select');
  const detectionModeDescription = document.querySelector('#detection-mode-description');
  ...
  function openDetectionChoices() {
    const modeLabel = captureMode === 'full-mouth' ? 'full-mouth' : 'single-image';
    detectionModeDescription.textContent = `Choose where the ${modeLabel} analysis runs.`;
